...
class Column {
	...
	public static function init(): void {
		...
		self::$render = static function (self $column, array|IEntity $values) {
			$value = self::getValueFromValues($column->getName(), $values);
			$value .= $column->getSuffix();
			return $value;
		};

		self::$renderDate = static function (self $column, array|IEntity $values) {
			$value = self::getValueFromValues($column->getName(), $values);
			try {
				if ($value === null || $value === '') {
					return null;
		...
		self::$renderDateTime = static function (self $column, array|IEntity $values) {
			$value = self::getValueFromValues($column->getName(), $values);
			try {
				if ($value === null || $value === '') {
					return null;
		...
		self::$renderNumber = static function (self $column, array|IEntity $values) {
			$value = self::getValueFromValues($column->getName(), $values);
		...
		self::$renderCurrency = static function (self $column, array|IEntity $values) {
			$value = self::getValueFromValues($column->getName(), $values);
			if ($value !== null) {
		...
		self::$renderBool = static function (self $column, array|IEntity $values) {
			$value = self::getValueFromValues($column->getName(), $values);
			if ($value === true) {
		...
		self::$renderEmail = static function (self $column, array|IEntity $values) {
			$value = self::getValueFromValues($column->getName(), $values);
			return "<a href=\"mailto:{$value}\" class=\"mail\" >{$value}</a>" . $column->getSuffix();
		};

		self::$renderPhone = static function (self $column, array|IEntity $values) {
			$value = self::getValueFromValues($column->getName(), $values);
			return "<a href=\"tel:{$value}\" class=\"mail\" >{$value}</a>" . $column->getSuffix();
		};

		self::$renderLink = static function (self $column, array|IEntity $values) {
			$value = self::getValueFromValues($column->getName(), $values);
			return "<a href=\"{$value}\" class=\"link\" target='_blank'>{$value}</a>" . $column->getSuffix();
		};

		self::$renderMonth = static function (self $column, array|IEntity $values) {
			$value = self::getValueFromValues($column->getName(), $values);
		...
		self::$renderButton = static function (self $column) {
	...
	public function render(array|IEntity $values): string {
		...
		if ($this->link) {
			...
			$this->link->processDynamicVars($values);
	 ...
	 * @param array|\QuickFeather\EntityManager\IEntity $values
	 * @return mixed
	 */
